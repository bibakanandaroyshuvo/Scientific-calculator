...
class _CalculatorScreenState extends State<CalculatorScreen> {
  ...
  void _togglePower() {
    setState(() {
      ...
      _output = _isPowerOn ? '0' : '';
    });
  }

  void _onPressed(String buttonText) {
    if (!_isPowerOn) return; // Ignore button presses if calculator is off
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('ScientificCalculator'),
      ),
      body: Column(
        children: <Widget>[
          Expanded(
          ...
          Row(
            children: <Widget>[
              Expanded(
                  child: _buildButton(
                      _isPowerOn ? 'C' : 'Power', 1, Colors.red, Colors.white)),
          ...
          Row(
            children: <Widget>[
              Expanded(
                  child: _buildButton(
                      _isPowerOn ? 'sin' : '', 1, Colors.white, Colors.black)),
              Expanded(
                  child: _buildButton(
                      _isPowerOn ? 'cos' : '', 1, Colors.white, Colors.black)),
              Expanded(
                  child: _buildButton(
                      _isPowerOn ? 'tan' : '', 1, Colors.white, Colors.black)),
              Expanded(child: _buildButton('/', 1, Colors.white, Colors.black)),
            ],
          ),
          Row(
          ...
          Row(
            children: <Widget>[
              ...
              Expanded(
                  child: _buildButton(
                      _isPowerOn ? '=' : '', 1, Colors.blue, Colors.white)),
              Expanded(
                  child: _buildButton(
                      _isPowerOn ? 'x²' : '', 1, Colors.white, Colors.black)),
            ],
          ),
        ],
      ),
